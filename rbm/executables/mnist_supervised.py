...
# by loading an unsupervised trained model
...
# and making predictions by using it and
...
with tb.SummaryWriter(tb_dir) as tb_writer:
    for iteration in range(300001):
        train_conditional_visible_ll = model.log_conditional_visible_likelihood(
            what, condition, mnist_all_possible_y
        )
        train_mean_conditional_visible_ll = torch.mean(train_conditional_visible_ll)
        tb_writer.add_scalar(
            "train_mean_conditional_visible_log_likelihood",
            train_mean_conditional_visible_ll,
            iteration,
        )
        train_geometric_mean_conditional_visible_l = torch.exp(
            train_mean_conditional_visible_ll
        )
        tb_writer.add_scalar(
            "train_geometric_mean_conditional_visible_likelihood",
            train_geometric_mean_conditional_visible_l,
            iteration,
        )
        optimizer.zero_grad()
        ...
        if iteration % 50 == 0:
            print(
                f"Iteration {iteration} done, train_geometric_mean_conditional_visible_likelihood = {train_geometric_mean_conditional_visible_l.item()}"
            )
